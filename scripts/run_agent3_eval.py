...
# Set to True to test Agent 3 WITH category filtering (the "after" state -
# passes each ticket's true_category, already normalized to match KB
# metadata display labels, as the where-filter). Set to False to reproduce
# the original "before" behavior (whole-collection search, no filter) for
# a side-by-side comparison. See kb_retrieval_agent.py::deflect()'s
# `category` parameter.
...
def evaluate_ticket(ticket: dict) -> dict:
    ...
    # ticket["true_category"] is already a KB-metadata display label
    # (e.g. "Wifi/Internet Support") after the label normalization pass -
    # no key conversion needed here, unlike the live pipeline where Agent 1
    # only produces internal keys (see kb_deflection_agent.py::build_kb_query).
    category = ticket.get("true_category") if USE_CATEGORY_FILTER else None

    # Run Agent 3 on this ticket
    result = deflect(ticket_text, category=category)
